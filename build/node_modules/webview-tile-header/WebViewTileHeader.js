...
const WebViewTileHeader = {
	...
	_detectCurrentPlatform() {
		const platform = (function getMobileOperatingSystem() {
	...
	addButton(settings, handler) {
		...
		if(settings.number != null) {
			...
			const value = parseInt(settings.number);
	...
	setBackButtonIcon(icon) {
		...
		const image = this._node.getElementsByClassName('npx-button-back-custom')[0];
		image.src = icon;
...
document.addEventListener('WebViewApiReady', (event) => {
	Nexpaq.Header = WebViewTileHeader;
	Moduware.v0.API.addEventListener('HardwareBackButtonPressed', () => {
...
document.addEventListener('DOMContentLoaded', (event) => {
	...
	const injector_js = atob(svg_injector);
	eval(injector_js);
});
...
WebViewTileHeader.addEventListener('BackButtonClicked', (event) => {
